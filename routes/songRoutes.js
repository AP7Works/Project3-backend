...
// Fetching data that changes on every upload
router.get('/songoftheday', async (req, res) => {
    try {
        // For example, we pick a random song from the database
        const songCount = await Song.countDocuments(); // Get the total number of songs
        const randomIndex = Math.floor(Math.random() * songCount); // Generate a random index
        const songOfTheDay = await Song.findOne().skip(randomIndex); // Skip to the random song

        if (!songOfTheDay) {
            return res.status(404).json({ success: false, message: 'No songs found' });
        }

        res.status(200).json({ success: true, data: songOfTheDay });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
